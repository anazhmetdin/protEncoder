...
def main():
    ...
    parser.add_argument("-p", "--GOpartioned", default="2000",
    ...
    parser.add_argument("-F", "--GOfilter", default="", help="file path of\
                        GO keys to be used for encoding filteration")
    parser.add_argument("-c", "--collection", default="", help="file path of\
                        collected protein annotations")
    ...
    if not (seqPath is None):
        ...
        if chopSize == -1:
            ...
            SeqIO.write(newFasta, outPrefix+"_part"+str(part)+".fasta",
                        'fasta')
            x = 1
            for name in glob.glob(outPrefix+"_part*"):
                oneHotencd.read(name)
                oneHotencd.encode()
                oneHotencd.dump(outPrefix+"_part"+str(x))
                x += 1
